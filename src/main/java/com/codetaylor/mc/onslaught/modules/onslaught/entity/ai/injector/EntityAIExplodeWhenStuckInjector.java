...
public class EntityAIExplodeWhenStuckInjector extends EntityAIInjectorBase {
  ...
  public void inject(EntityLiving entity, NBTTagCompound tag) {
    ...
    ModuleOnslaughtConfig.CustomAI.ExplodeWhenStuck config =
        ModuleOnslaughtConfig.CUSTOM_AI.EXPLODE_WHEN_STUCK;
    boolean sightRequired =
        this.getBoolean(aiTag, Tag.AI_PARAM_SIGHT_REQUIRED, config.DEFAULT_SIGHT_REQUIRED);
    boolean rangeRequired =
        this.getBoolean(aiTag, Tag.AI_PARAM_RANGE_REQUIRED, config.DEFAULT_RANGE_REQUIRED);
    ...
    int explosionDelayTicks =
        this.getInteger(
            aiTag, Tag.AI_PARAM_EXPLOSION_DELAY_TICKS, config.DEFAULT_EXPLOSION_DELAY_TICKS);
    float explosionStrength =
        (float)
            this.getDouble(
                aiTag, Tag.AI_PARAM_EXPLOSION_STRENGTH, config.DEFAULT_EXPLOSION_STRENGTH);
    boolean explosionCausesFire =
        this.getBoolean(
            aiTag, Tag.AI_PARAM_EXPLOSION_CAUSES_FIRE, config.DEFAULT_EXPLOSION_CAUSES_FIRE);
    boolean explosionDamaging =
        this.getBoolean(aiTag, Tag.AI_PARAM_EXPLOSION_DAMAGING, config.DEFAULT_EXPLOSION_DAMAGING);

    entity.tasks.addTask(
        priority,
        new EntityAIExplodeWhenStuck(
            entity,
            sightRequired,
            rangeRequired,
            rangeMin,
